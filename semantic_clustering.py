...
def process_notes(root_directory):
    """Main function to process notes, extract topics, and create links."""
    create_database()
    ...
    for root, _, files in os.walk(root_directory):
        for file in files:
            if file.endswith(".md"):
                ...
                clean_content = remove_whitespace(clean_content)
                tokens = tokenize_text(clean_content)
                tokens = remove_stopwords(tokens)
                tokens = lemmatize_text(tokens)

                # Calculate hash
                note_hash = calculate_hash(clean_content)
                stored_hash = get_note_hash_from_db(note_path)

                # Skip if note hasn't changed
                if note_hash == stored_hash:
                    continue

                # Topic Modeling
                topics, lda_model, dictionary = lda_topic_modeling([tokens])

                # Generate Embedding
                embedding = generate_embedding(clean_content, embedding_model)
                embedding_blob = embedding.tobytes()

                # Store data in the database
                store_note_data(note_path, note_hash, str(topics), embedding_blob)

    visualize_topics_heatmap(topics, lda_model, dictionary)

    # Link creation
    all_embeddings = get_all_embeddings_and_paths()
    for note_path, embedding in all_embeddings:
        similar_notes = find_similar_notes(embedding, all_embeddings)

        # Add links to note content
        with open(note_path, "r", encoding="utf-8") as f:
            content = f.read()

        new_links = "\n".join(
            [f"[[{os.path.basename(path)}]]" for path in similar_notes]
        )
